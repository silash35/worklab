...
def plot_space(Cv1, Cv2, Z, filename: str | None = None):
    plt.figure(figsize=(16 * 0.75, 9 * 0.75), layout="constrained", dpi=default_dpi)
...
def plot_space_3d(Cv1, Cv2, Z, filename: str | None = None):
    fig = go.Figure(
        data=[go.Surface(z=Z ** (1 / 8), x=Cv1, y=Cv2, colorscale="Viridis")],
    )
...
def plot_population(Cv1, Cv2, Z, population: Population, filename: str | None = None):
    ...
    for ind in population:
        plt.scatter(ind["chromosome"][0], ind["chromosome"][1], color="black")

    plt.scatter(
        best_ind["chromosome"][0],
        best_ind["chromosome"][1],
        marker="*",
        color="gold",
        linewidths=1.5,
        edgecolor="black",
